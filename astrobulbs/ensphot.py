...
def ens_lc(in_file,target,date,scope,all_lc='yes',n_min=20,s_width=0.25,color=False):
    ...
    color : bool, optional
        Default is False.
        Specifies whether the fit is made with color information. 
        Code functions the same regardless.
    ...
    #else:
    #    keys,ee,ss,a,b,mag_es,w_es,x,em,emerr,m0,m0err,k,T_BR=ens_fill_color(matches)
...
def add_color_match(match_file,output='keep'):
    '''
    A program that adds the Gaia DR3 Bp-Rp color to a matched dictionary.
    ...
    ----------
    match_file : str
        The file name of the pickle file containing the matched dictionary.

    output : str, optional
        Accepted values are 'keep' and 'delete'.
        The default is 'keep'.
        The option to delete stars from the dictionary 
        that do not have a match in the Gaia DR3 catalog, or the keep
        the star in the dictionary and set the color to 0.0.

    Returns
    -------
    matches : dictionary
        The matched dictionary with the Gaia DR3 Bp-Rp color added to each 
        star.
    '''

    from astroquery.vizier import Vizier  
    dr3 = Vizier(columns=["*","+_r","_RAJ2000","_DEJ2000","Epoch","Plx","BPmag","RPmag"],catalog=['I/355/gaiadr3'])

    matches=pickle.load(open(match_file,'rb'))
    keys=list(matches.keys())
    ...
    for i in range(len(keys)):
        ...
        if matches[keys[i]][0][13] == 1:
            comp[i]=1
        coord = SkyCoord(ra=ra[i]*u.degree, dec=dec[i]*u.degree, frame='icrs')

        result = dr3.query_region(coord,radius='0d0m3s')
        if len(result) == 0:
            if output == 'delete':
                print('No match found for '+keys[i]+', deleting the star')
                del matches[keys[i]]
            if output == 'keep':
                print('No match found for '+keys[i]+', setting color to 0')
                GBp = 0.0
                GRp = 0.0
                for j in range(len(matches[keys[i]])):
                    matches[keys[i]][j].append(GBp-GRp)
        else:
            GBp = result['I/355/gaiadr3']['BPmag'][0]
            GRp = result['I/355/gaiadr3']['RPmag'][0]
            if type(GBp) == np.ma.core.MaskedConstant:
                if output == 'delete':
                    print('No match found for '+keys[i]+', deleting the star')
                    del matches[keys[i]]
                if output == 'keep':
...
def ens_fill_color(matches):

    '''A program that creates and solves the system of linear equations 
    outlined in Honeycutt 1992 (i.e. populating matricies), updated to include 
    an airmass term and a color transformation. Reads in the
    pyred.ens_match output dictionary. The system of equations are setup to 
    minimize the standard deviation of the mean magnitude of each star 
    simultaneously. The solution provides the exposure magnitudes for each 
    frame and the mean magnitude for each star.
    ...
    k : float
        The airmass coefficient.

    T_BR : float
        The color transformation coefficient.
    ...
    ee=np.array([row[0] for row in matches[keys[0]]])
    ss=np.array(keys,dtype=str)
    a=np.zeros((ee.size+ss.size+2,ee.size+ss.size+2))
    b=np.zeros((ee.size+ss.size+2))
    mag_es=np.zeros((ee.size,ss.size))
    w_es=np.zeros((ee.size,ss.size))
    air_e=np.zeros(ee.size)
    color_s=np.zeros(ss.size)

    #fill arrays
    for i in range(ee.size):
        air_e[i]=((matches['Target'])[i])[16]
        for j in range(ss.size):
            w1=((matches[ss[j]])[i])[12]
            w2=((matches[ss[j]])[i])[13]
            w3=((matches[ss[j]])[i])[14]
    ...
    for i in range(ee.size):
        ...
        a[ee.size+ss.size,i]= air_e[i]*np.sum(w1*w2*w3*w4)
        a[ee.size+ss.size+1,i]= np.sum(color_s*w1*w2*w3*w4)
        
        a[i,ee.size+ss.size]= air_e[i]*np.sum(w1*w2*w3*w4)
        a[i,ee.size+ss.size+1]= np.sum(color_s*w1*w2*w3*w4)
    ...
    for i in range(ss.size):
        ...
        a[ee.size+ss.size,ee.size+i]= np.sum(air_e*(w1*w2*w3*w4))
        a[ee.size+ss.size+1,ee.size+i]= color_s[i]*np.sum(w1*w2*w3*w4)

        a[ee.size+i,ee.size+ss.size]= np.sum(air_e*(w1*w2*w3*w4))
        a[ee.size+i,ee.size+ss.size+1]= color_s[i]*np.sum(w1*w2*w3*w4)

        b[i+ee.size]=np.sum(w1*w2*w3*w4*mag)

    b[-2] = np.sum((mag_es*w_es).T*air_e)

    b[-1] = np.sum(mag_es*w_es*color_s)

    #diagonal terms
    a[-2,-2] = np.sum(air_e**2*w_es.T)
    a[-1,-1] = np.sum(color_s**2*w_es)

    #off diagonal terms
    a[-2,-1] = np.sum((w_es*color_s).T*air_e)
    a[-1,-2] = np.sum((w_es*color_s).T*air_e)

    #Removing the matrix rows and columns and vector component to make em[1] = 0
    #This necessarily sets m0[brightest star] to the instrumental magnitude measured 
    #in the first image, which we set to 0 in the ems_match_zp function.
    b = b[1:]
    a = a[1:,1:]
    ...
    em=x[:ee.size-1]
    em = np.concatenate([[0],em])

    print('ee,ss',mag_es.shape)
    ...
    #This isn't quite right, but I don't actually need it
    
    emerr=np.sqrt(ss.size*np.sum([(mag_es[:,d]-em-m0[d]-k*air_e-T_BR*color_s[d])**2 * w_es[:,d] 
                                  for d in range(ss.size)],axis=0) / 
                  ((ss.size-1.0)*np.sum(w_es,axis=1)))

    #changing this from the formal uncertainty to the standard deviation of the
    #ens light curve for this star. Heads up, the formal uncertainty below is
    #not correct.
    m0err=np.sqrt(ee.size*np.sum([(mag_es[d,:]-em[d]-m0-k*air_e[d]-T_BR*color_s)**2 * w_es[d,:] 
                                  for d in range(ee.size)],axis=0) /
                  ((ee.size-1.0)*np.sum(w_es,axis=0)))
    #m0err = np.zeros(ss.size)
    #for i in range(ss.size):
    #    m0err[i] = np.std(mag_es[:,i] - em - k*air_e - T_BR*color_s[i])

    #not returning these for now, probably don't need them
    #kerr = 
    #T_BRerr = 

    return keys,ee,ss,a,b,mag_es,w_es,x,em,emerr,m0,m0err,k,T_BR,air_e,color_s


def ens_fill_air(matches):
    ...
    ee=np.array([row[0] for row in matches[keys[0]]])
    ss=np.array(keys,dtype=str)
    a=np.zeros((ee.size+ss.size+1,ee.size+ss.size+1))
    b=np.zeros((ee.size+ss.size+1))
    mag_es=np.zeros((ee.size,ss.size))
    w_es=np.zeros((ee.size,ss.size))
    air_e=np.zeros(ee.size)
    ...
    b[-1] = np.sum((mag_es*w_es).T*air_e)

    #diagonal term
    a[-1,-1] = np.sum(air_e**2*w_es.T)
    
    
    #Removing the matrix rows and columns and vector component to make em[1] = 0
    #This necessarily sets m0[brightest star] to the instrumental magnitude measured 
    #in the first image, which we set to 0 in the ems_match_zp function (modulo the airmass term).
    b = b[1:]
    a = a[1:,1:]
    ...
    emerr=np.sqrt(ss.size*np.sum([(mag_es[:,d]-em-m0[d]-k_air*air_e)**2 * w_es[:,d] 
                                  for d in range(ss.size)],axis=0) / 
                  ((ss.size-1.0)*np.sum(w_es,axis=1)))
    ...
    #ens light curve for this star. Heads up, the formal uncertainty below is
    #not correct.
    m0err=np.sqrt(ee.size*np.sum([(mag_es[d,:]-em[d]-m0-k_air*air_e[d])**2 * w_es[d,:] 
                                  for d in range(ee.size)],axis=0) /
                  ((ee.size-1.0)*np.sum(w_es,axis=0)))
    #m0err = np.zeros(ss.size)
    #for i in range(ss.size):
    ...
    return keys,ee,ss,a,b,mag_es,w_es,x,em,emerr,m0,m0err,k_air,air_e


def ens_match_zp(in_file,tra,tdec,scope='lcogt',out_file='default',d_max=1.0,nn_dist=20,edge_buffer=50, min_apps=30,mag_ulim=50.0,mag_llim=-50.0,magerr_ulim=0.5):
    ...
    if cont == 'u':
        ...
        #adjust the zero-point for the magnitudes and the errors
        print('Adjusting the zero-point')
        ss=np.array(keys,dtype=str)
        fs_1 = np.zeros(ss.size)
        fs_err_1 = np.zeros(ss.size)
        for i in range(ss.size):
            fs_1[i] = matches[keys[i]][0][3]
            fs_err_1[i] = matches[keys[i]][0][4]

        f_ref = np.max(fs_1)

        for i in range(ss.size):
            for j in range(ee.size):
                matches[keys[i]][j][5] = -2.5*np.log10(matches[keys[i]][j][3]/f_ref)
                matches[keys[i]][j][6] = (2.5/np.log(10))*(matches[keys[i]][j][4]/matches[keys[i]][j][3])
...
def ens_fill_zp(matches):
    ...
    #removing the matrix rows and columns to make em[1] = 0
    ...
    #m0err = np.zeros(ss.size)
    #for i in range(ss.size):
    #    m0err[i] = np.std(mag_es[:,i] - em)

    a_mean = np.zeros(ss.size)
    a_std = np.zeros(ss.size)
    for i in range(ss.size):

        g=np.array([row[14]*row[12] for row in matches[ss[i]]])>0.1

        ss_mag=np.array([row[5] for row in matches[ss[i]]])
        ss_mag = ss_mag[g]

        ss_ens_mag=(ss_mag-em[g])

        a_mean[i] = np.mean(ss_ens_mag)
        a_std[i] = np.std(ss_ens_mag)

    return keys,ee,ss,a,b,mag_es,w_es,x,em,emerr,m0,m0err,a_mean,a_std


def ens_lc_air(in_file,target,date,scope,all_lc='yes',n_min=20,s_width=0.25,air=False):
    ...
    if air == False:
        keys,ee,ss,a,b,mag_es,w_es,x,em,emerr,m0,m0err=ens_fill_alt(matches)
        # pyred.ens_fill_alt() can be found below. It is a program to create the 
        # system of linear equations (matrix) from the pyred.ens_match() 
        # dictionary and solve them in the Honeycutt 1992 formalism.
    else:
        keys,ee,ss,a,b,mag_es,w_es,x,em,emerr,m0,m0err,k_air,air_e=ens_fill_air(matches)
    ...
    def press_key(event,plotted=plotted,p=p,lc_p=lc_p,lc_id=lc_id,em_p=em_p,
        ...
        if plotted[0] == 'mags':
            ...
            if event.key == 'e':
                llc_mag=np.array([row[5] for row in matches[(ss[d == np.min(d)])[0]]]- em - k_air*air_e) - mag_shift
    ...
    if redo == 'n':
        ...
        t_air=np.array([row[16] for row in matches['Target']]) - 1
        t_air=t_air[(g == True) & (ss_eff > 0)]
        ...
        ens_mag=(t_mag-t_em-k_air*t_air)
        ens_mag_err=np.sqrt(t_magerr**2+((t_emerr)**2 / t_ss_eff))
        ...
        if output == 'y':
            ...
            for i in range(len(ss[comps])):
                ...
                ss_air=np.array([row[16] for row in matches[ss[comps][i]]]) - 1
                ss_air=ss_air[(g == True) & (ss_eff > 0)]
                ...
                ss_ens_mag=(ss_mag-ss_em-k_air*ss_air)
...
def ens_lc_zp(in_file,target,date,scope,all_lc='yes',n_min=20,s_width=0.25):
    ...
    #if color == False:
    keys,ee,ss,a,b,mag_es,w_es,x,em,emerr,m0,m0err,a_mean,a_std=ens_fill_zp(matches)
        # pyred.ens_fill() can be found below. It is a program to create the 
        # system of linear equations (matrix) from the pyred.ens_match() 
        # dictionary and solve them in the Honeycutt 1992 formalism.
    #else:
    ...
    def plot_mags(m0,m0err,p,n_valid,target_err):
        ax.plot(m0,m0err,'bo',markersize=0.5)
        ax.plot(m0[p==1],m0err[p==1],'bo',markersize=5)
        ax.plot(m0[p==0],m0err[p==0],'rx',markersize=6,mew=2)
        ax.plot(m0[keys == 'Target'],m0err[keys == 'Target'],'ro',ms=5)
        ax.axis([np.min(m0)-np.abs(np.max(m0)-np.min(m0))*0.03,
                 np.max(m0)+np.abs(np.max(m0)-np.min(m0))*0.03,
                 np.min(m0err)-np.abs(np.max(m0err)-np.min(m0err))*0.03,
        ...
        ax.set_xlabel('Mean Mag')
        ax.set_ylabel('Standard Deviation of Mean Mag')
        ax.set_title('Mean Magnitude Plot: Formal Version')
        plt.draw()
    ...
    # Function to make PLOT IV
    def r_plot(a_mean,a_std,p,m0,m0err):
        ax.set_title('Error Analysis Plot')
        ax.plot(m0,a_std/m0err,'bo',ms=0.5)
        ax.plot(m0[p==1],(a_std/m0err)[p==1],'bo',ms=5)
        ax.plot(m0[p==0],(a_std/m0err)[p==0],'rx',ms=6,mew=2)
        ax.axis([np.min(m0)-np.abs(np.max(m0)-np.min(m0))*0.03,
                 np.max(m0)+np.abs(np.max(m0)-np.min(m0))*0.03,
                 np.min(a_std/m0err)-np.abs(np.max(a_std/m0err)-np.min(a_std/m0err))*0.03,
                 np.max(a_std/m0err)+np.abs(np.max(a_std/m0err)-np.min(a_std/m0err))*0.03])
        ax.plot(m0[keys == 'Target'],(a_std/m0err)[keys == 'Target'],'ro',ms=5)
        ax.axvline(m0[keys=='Target'],ls='--',color='green')
        ax.axhline(1.0,ls='--',color='black')

        ax.set_xlabel('Mean Mag')
        ax.set_ylabel(r'$\mathregular{\sigma_{empirical} / \sigma_{formal}}$')
        plt.draw()

    # Function to make PLOT V
    def std_plot(m0,m0err,p):
        ax.plot(m0,m0err,'bo',markersize=0.5)
        ax.plot(m0[p==1],m0err[p==1],'bo',markersize=5)
        ax.plot(m0[p==0],m0err[p==0],'rx',markersize=6,mew=2)
        ax.plot(m0[keys == 'Target'],m0err[keys == 'Target'],'ro',ms=5)
        ...
        ax.set_xlabel('Mean Mag')
        ax.set_ylabel('Standard Deviation of Mean Mag')
        ax.set_title('Mean Magnitude Plot: Empirical Version')
        plt.draw()


    # Interactive Plot Commands
    def press_key(event,plotted=plotted,p=p,lc_p=lc_p,lc_id=lc_id,em_p=em_p,
                  lc_hjd=lc_hjd,lc_mag=lc_mag,lc_magerr=lc_magerr,a_mean=a_mean,a_std=a_std):
        # PLOT I Key Commands - Mean Magnitude Plot
        ...
        if plotted[0] == 'mags':
            ...
            if event.key == 'r':
                la_mean = np.zeros(ss.size)
                la_std = np.zeros(ss.size)
                for i in range(ss.size):
                    ...
                    la_mean[i] = np.mean(ss_ens_mag)
                    la_std[i] = np.std(ss_ens_mag)
                ...
                plt.cla()
                r_plot(a_mean,a_std,p,m0,m0err)
                plotted[0]='ratio'
        ...
        # PLOT IV Key Commands - Standard Deviation Ratio Plot
        if plotted[0] == 'ratio':
            x_norm=np.abs(np.max(m0)-np.min(m0))*0.8
            y_norm=np.abs(np.max(a_std/m0err)-np.min(a_std/m0err))
            # Normalization takes into account axis scales and apsect ratio
            d=np.sqrt((event.xdata/x_norm-m0/x_norm)**2+
                      (event.ydata/y_norm-(a_std/m0err)/y_norm)**2)
        ...
        # PLOT V Key Commands - Standard Deviation Plot
        if plotted[0] == 'std':
            x_norm=np.abs(np.max(a_mean)-np.min(a_mean))*0.8
            y_norm=np.abs(np.max(a_std)-np.min(a_std))
            # Normalization takes into account axis scales and apsect ratio
            d=np.sqrt((event.xdata/x_norm-a_mean/x_norm)**2+
                      (event.ydata/y_norm-a_std/y_norm)**2)
    ...
    if redo == 'n':
        ...
        print('adjusting error')
        da_mean = np.min((a_mean[p==1][np.argsort(a_mean[p==1])] - np.roll(a_mean[p==1][np.argsort(a_mean[p==1])],1))[1:])
        x_new = np.arange(np.min(a_mean[p==1]),np.max(a_mean[p==1]),da_mean)
        flinear = interpolate.interp1d(a_mean[p==1][np.argsort(a_mean[p==1])],a_std[p==1][np.argsort(a_mean[p==1])])
        y_linear = flinear(x_new)
        y_smooth = gaussian_filter(y_linear,s_width/da_mean)
        
        x_smooth = np.concatenate([[np.min(x_new)-3],x_new,[np.max(x_new)+3]])
        y_smooth = np.concatenate([[np.min(y_smooth)],y_smooth,[np.max(y_smooth)]])

        #if np.min(ens_mag) < np.min(a_mean[p==1]):
        #    x_smooth = np.concatenate([[np.min(ens_mag)],x_new])
        #    y_smooth = np.concatenate([[np.min(y_smooth)],y_smooth])
        #elif (np.max(ens_mag) > np.max(a_mean[p==1])):
        #    x_smooth = np.concatenate([x_new,[np.max(ens_mag)]])
        #    y_smooth = np.concatenate([y_smooth,[np.max(y_smooth)]])
        #else:
        #    x_smooth = x_new

        #old, switched to the empirical values
        #dm0 = np.min((m0[p==1][np.argsort(m0[p==1])] - np.roll(m0[p==1][np.argsort(m0[p==1])],1))[1:])
        #x_new = np.arange(np.min(m0[p==1]),np.max(m0[p==1]),dm0)
        #flinear = interpolate.interp1d(m0[p==1][np.argsort(m0[p==1])],m0err[p==1][np.argsort(m0[p==1])])
        #y_linear = flinear(x_new)
        ...
        if output == 'y':
            ...
            for i in range(len(ss[comps])):
                ...
                try:
                    ss_emp_std = f_lin_smooth(ss_ens_mag)
                except:
                    below = ss_ens_mag < np.min(x_smooth)
                    to_interp = (ss_ens_mag >= np.min(x_smooth)) & (ss_ens_mag <= np.max(x_smooth))
                    above = ss_ens_mag > np.max(x_smooth)

                    ss_emp_std = np.ones(ss_ens_mag.size)
                    ss_emp_std[to_interp] = f_lin_smooth(ss_ens_mag[to_interp])

                    if np.sum(above) > 0:
                        ss_emp_std[above] = np.max(y_smooth)
                    if np.sum(below) > 0:
                        ss_emp_std[below] = np.min(y_smooth)

                ss_ens_mag_err = np.sqrt(ss_ens_mag_err**2 + ss_emp_std**2)
